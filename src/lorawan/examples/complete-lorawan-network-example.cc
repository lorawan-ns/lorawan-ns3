...
void
Teste1 (Ptr<Packet const> packet, uint32_t systemId)
{
	if(verbose){
	 NS_LOG_INFO ("A packet arrived at the gateway under sensitivity at gateway " << systemId);
 }
...
//	loss->SetReference (1, 8.1);
 ...
 macHelper.SetSpreadingFactorsUp (endDevices, gateways, channel);
	...
	*************/
	...
	std::cout << nDevices << " " << double(nDevices)/simulationTime << " " << receivedProb/(simulationTime/appPeriodSeconds) << " " << interferedProb /(simulationTime/appPeriodSeconds)<< " " << noMoreReceiversProb /(simulationTime/appPeriodSeconds)<< " " << underSensitivityProb /(simulationTime/appPeriodSeconds)<< " " << receivedProbGivenAboveSensitivity << " " << interferedProbGivenAboveSensitivity << " " << noMoreReceiversProbGivenAboveSensitivity<< std::endl;
	received=0;
	interfered=0;
	noMoreReceivers=0;
	underSensitivity=0;
