...
{-132.0, -134.5, -137.0, -139.5, -142.0, -144.5};
                              ...
                              uint8_t sf, Time duration, double frequencyMHz)
{
  ...
  //NS_LOG_DEBUG (this <<" -- "<< packet << rxPowerDbm << duration << frequencyMHz);
                            ...
                            Ptr<LoraInterferenceHelper::Event> event)
{
  ...
  if (packetDestroyed != uint8_t (0))
    {
      NS_LOG_DEBUG ("packetDestroyed by " << unsigned(packetDestroyed) << " " << event);
